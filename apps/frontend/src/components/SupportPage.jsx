...
export default function SupportPage() {
  ...
  useEffect(() => { api.get("/citizen/me").then(({data}) => { setUser(data); return load(); }).catch(() => setNotice("Sign in to create and track support tickets.")); }, []);
  ...
  async function consent(value) {
    try { await api.put("/citizen/sms-consent", {consent:value}); setNotice(value ? "You opted in to website awareness SMS. No message has been sent." : "You opted out of future SMS outreach."); }
  ...
  return <MainLayout><section className="mx-auto max-w-4xl px-5 py-12">
    ...
    <div className="my-5 rounded-xl border bg-white p-5"><h2 className="text-lg font-bold">Contact the SchemeSaathi team</h2><p className="mt-2"><a className="underline" href="mailto:srivastava2722@gmail.com">srivastava2722@gmail.com</a></p><p className="mt-2"><a className="underline" href="tel:+919653031393">+91 96530 31393</a></p><p className="mt-2 text-sm text-slate-500">For website and scheme-discovery help. Response availability depends on the project team.</p></div>
    ...
    <section className="my-6 rounded-xl border bg-white p-6"><h2 className="text-xl font-bold">Website awareness SMS</h2><p className="my-3">Choose whether the project team may send a website link to your registered mobile number. You can opt out here anytime.</p>
      <button onClick={() => consent(true)} className="mr-3 rounded border p-3">Opt in to SMS</button><button onClick={() => consent(false)} className="rounded border p-3">Opt out</button></section>
    ...
    {tickets.map(ticket => <article key={ticket.id} className="my-4 rounded-xl border bg-white p-5">
      <h3 className="font-bold">{ticket.subject}</h3><p className="text-xs text-slate-500">{ticket.id} · {ticket.status}</p><p className="my-3 whitespace-pre-wrap">{ticket.message}</p>
