...
def methods() -> dict:
    return {"email": email_client.email_ready(), "mobile": firebase_phone.ready() or sms_client.otp_sms_ready(), "mobile_provider": "firebase" if firebase_phone.ready() else "msg91"}


def destination(user: User, channel: str) -> str:
    return user.email if channel == "email" else (user.mobile or "")


def masked(contact: str, channel: str) -> str:
    if channel == "mobile":
        return "******" + contact[-4:]
...
def active_challenge(row: LoginChallenge, user: User | None) -> bool:
    return bool(user and user.is_active and user.auth_version == row.auth_version
                ...
                and hmac.compare_digest(row.contact_hash, secret_hash("login-contact", row.channel, destination(user, row.channel))))
...
async def request_login(db: AsyncSession, user: User, channel: str, recaptcha_token: str | None = None, language: str = "en") -> dict:
    ...
    contact = destination(user, channel)
    if not methods().get(channel) or not contact:
        raise HTTPException(503, "Sign-in verification for this contact method is temporarily unavailable. Choose the other method or try again later.")
    if channel == "mobile" and firebase_phone.ready() and not recaptcha_token:
    ...
    db.add(LoginChallenge(id=challenge_id, user_id=user.id, channel=channel, auth_version=user.auth_version,
        ...
        contact_hash=secret_hash("login-contact", channel, contact), otp_hash=("firebase" if channel == "mobile" and firebase_phone.ready() else secret_hash("login-otp", challenge_id, code)),
    ...
    try:
        ...
        elif firebase_phone.ready():
            delivered = await firebase_phone.send_code(db, challenge_id, contact, recaptcha_token, language)
        else:
            delivered = await sms_client.send_reset_otp(contact, code)
    except Exception:
    ...
    if not delivered:
        ...
        raise HTTPException(503, "The verification code could not be sent. Wait 60 seconds and try again, or use the other contact method.")
    payload = {"challenge_id": challenge_id, "channel": channel, "destination": masked(contact, channel),
              "expires_in": OTP_TTL, "resend_after": RESEND_DELAY, "message": "A sign-in verification code has been sent to your registered contact."}
    return payload


async def resend_login(db: AsyncSession, challenge_id: str, recaptcha_token: str | None = None, language: str = "en") -> dict:
    ...
    if not active_challenge(row, user):
        raise expired()
    return await request_login(db, user, row.channel, recaptcha_token, language)
...
async def verify_login(db: AsyncSession, challenge_id: str, code: str) -> User:
    ...
    if not attempt:
        await db.rollback()
        raise expired()
    valid = await firebase_phone.verify_code(db, challenge_id, user.mobile, code) if attempt.otp_hash == "firebase" else hmac.compare_digest(attempt.otp_hash, secret_hash("login-otp", challenge_id, code))
    if not valid:
        await db.commit()
